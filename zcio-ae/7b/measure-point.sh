...
# all connected NVMe/TCP devices and record the STEADY-STATE summed TX throughput
# over both initiator NICs (writes flow stream5 -> rapids0, so the initiator TX
...
[[ -n "$NIC_95" && -n "$NIC_96" ]] || { echo "ERROR: data NICs not configured (run stream5-net.sh)"; exit 1; }
TX95="/sys/class/net/$NIC_95/statistics/tx_bytes"
TX96="/sys/class/net/$NIC_96/statistics/tx_bytes"
...
echo "[measure] label=$LABEL  ${#DEVS[@]} devs: ${DEVS[*]}  NICs: .95=$NIC_95 .96=$NIC_96"
...
try:
    ...
    mbps = kiops = clat = 0.0
print("  net TX %s (both NICs): %s GB/s  (CoV %s%%)   (fio %.0f MiB/s, %.1f kIOPS, clat %.1f us)"
