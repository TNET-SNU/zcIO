#!/bin/bash
# stream6-net.sh (fig-7b) — SECOND initiator data-plane setup. stream6 drives the
# 4 subsystems on the .95 target NIC (10.3.95.10), so it only needs its one data
# NIC on the 95 subnet. Same recipe as stream5-net.sh (reload mlx5, bring link up
# at MTU 1500, enable hw-gro, bump to 9000, set 8192 rings).
#
#   ens2np0 -> 10.3.95.2/24   (reaches target ens17np0 @ 10.3.95.10)
#
#   Usage:  sudo ./stream6-net.sh
set -uo pipefail
...
NICS=(
  "ens2np0|10.3.95.2/24|10.3.95.10"
)
MTU=9000

reload_mlx5() {
  echo "[stream6-net] reloading mlx5 (unload mlx5_ib+mlx5_core -> load core, ib)"
...
echo "[stream6-net] interfaces:"
ip -br addr show | grep -E "10\.3\.95\.2" || true

sleep 1
for entry in "${NICS[@]}"; do
  IFS='|' read -r iface cidr tgt <<< "$entry"
  if ping -c1 -W1 -I "$iface" "$tgt" >/dev/null 2>&1; then
    echo "[stream6-net] $iface -> $tgt : OK"
  else
    echo "[stream6-net] $iface -> $tgt : UNREACHABLE (check cabling/target NIC)"
  fi
done
