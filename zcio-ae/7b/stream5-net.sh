#!/bin/bash
# stream5-net.sh (fig-7d) — initiator data-plane setup. Reloads mlx5 first so
...
reload_mlx5() {
  ...
  # Disconnect all NVMe/TCP first: tearing the NIC down with live connections on
  # it leaves zombie controllers, and reconnecting then fails with EXDEV
