...
# FMT_THRESHOLD_BYTES, for a clean benchmark start. Model-gated: the boot drive
# (970 EVO) and any other model are never touched. Skip with SKIP_FORMAT=1.
format_dirty_9100() {
...
main() {
  ...
  # clean start: format any 9100 PRO that still has data, before exporting it
