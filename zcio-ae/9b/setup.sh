#!/bin/bash
# setup.sh — fig-9b (MLPerf Storage + RocksDB) common environment setup.
#
# Brings the testbed to the state every fig-9b workload assumes, and STOPS there:
#
...
RAPIDS0_DIR="${RAPIDS0_DIR:-$HOME/zcio-ae-9b}"
SSH="ssh -o ConnectTimeout=10 -o ServerAliveInterval=5 $RAPIDS0"

# fig-9b: 4 drives as 4 namespaces under ONE subsystem -> ONE controller / TCP
...
echo
echo "############################################################"
echo "# fig-9b setup complete  (COMPLETE DEFAULT-LINUX baseline)."
