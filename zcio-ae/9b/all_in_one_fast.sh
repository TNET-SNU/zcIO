#!/bin/bash
# all_in_one_fast.sh — figure 9b FAST subset: only 3 (config, cores) points.
#
#       default @ 6 cores  ·  default @ 10 cores  ·  zcIO @ 6 cores
#
# Same flow as all_in_one.sh but with a PER-CONFIG core list. Each config still
# datagens once (a config change reconnects + re-mkfs's the disks, wiping data),
# so this runs 2 datagens total — default (cores 6,10) and zcIO (core 6).
#
# Prereqs (one-time): ../deploy.sh   (/opt/mlperf-env is pre-provisioned & shared on this machine)
#   Usage:  ./all_in_one_fast.sh
set -uo pipefail
...
# per-config online-core list (THIS is the only real difference from all_in_one.sh)
...
OUTDIR="${OUTDIR:-results-fast}"; mkdir -p "$OUTDIR"; OUTDIR="$(cd "$OUTDIR" && pwd)"
SETTLE="${SETTLE:-3}"
settle() { sleep "$SETTLE"; }
...
echo
echo "[all_in_one_fast] done. CSVs + plot under: $OUTDIR/"
